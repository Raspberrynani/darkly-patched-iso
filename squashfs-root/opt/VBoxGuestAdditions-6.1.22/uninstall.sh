...
# Read routines.sh
if ! test -r "/opt/VBoxGuestAdditions-6.1.22/routines.sh"; then
...
echo 1>&2 "Removing installed version 6.1.22 of VirtualBox Guest Additions..."
...
if test -r "/opt/VBoxGuestAdditions-6.1.22"/init/vboxadd-service; then
...
if test -r "/opt/VBoxGuestAdditions-6.1.22"/init/vboxadd; then
    stop_init_script vboxadd 2>> "$LOGFILE"
    test -n "$NO_CLEANUP" || "/opt/VBoxGuestAdditions-6.1.22"/init/vboxadd cleanup 2>> "$LOGFILE"
...
for CUR_MODULE in /opt/VBoxGuestAdditions-6.1.22/installer/module-autologon
    do
        . "$CUR_MODULE"
    done
